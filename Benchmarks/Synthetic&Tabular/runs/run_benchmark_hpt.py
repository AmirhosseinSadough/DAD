...
from additional_methods.wrappers.DAD import DAD
...
method_parameters = {
        "DAD":{"mode": ["manual"], "lr":[0.8, 0.2, 8e-2, 2e-2, 8e-3, 2e-3, 8e-4, 2e-4, 8e-5, 2e-5, 8e-6, 8e-7], "mom_score":[0.25], "win_size":[0], "normalize":[False]},      
...
for dataset_name in dataset_names:
    ...
    try:
        ...
        if not os.path.exists(besthyp_dir) or os.path.getsize(besthyp_dir) == 0:
            ...
            with open(besthyp_dir, 'r') as f:
                try:
                    # 1. Parse the outer dictionary string
                    outer_dict = ast.literal_eval(f.read())
                    raw_hyp = outer_dict['best_hyperparameter']
                    
                    # 2. Parse the inner dictionary string into a real Python dict
                    if isinstance(raw_hyp, str):
                        best_dict = ast.literal_eval(raw_hyp)
                    else:
                        best_dict = raw_hyp
                        
                    print(f"load again: {method_name} Best hyperparameters for {dataset_name} is {best_dict}")
                    
                    # 3. Store it as a list containing the dictionary object
                    method_parameters_test[method_name] = [best_dict]
        ...
        for hyperparameter_setting in hyperparameter_list:
            ...
            if os.path.exists(target_file_name) and os.path.getsize(target_file_name) > 0:
                ...
                if method_name in ["DeepSVDD", "sb-DeepSVDD"]:
                    ...
                    elif method_name in ["DynamicHBOS", "gen2out"]:
                        hyperparameter_setting = '{}'
                        hyperparameter_setting = ast.literal_eval(hyperparameter_setting)
                    else:
                        # SAFEGUARD: Only parse if it's actually a string representation
                        if isinstance(hyperparameter_setting, str):
                            hyperparameter_setting = ast.literal_eval(hyperparameter_setting)
                        else:
                            hyperparameter_setting = hyperparameter_setting  # Leave it as the clean dict it already is
